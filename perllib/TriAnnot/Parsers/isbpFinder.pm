...
package TriAnnot::Parsers::isbpFinder;
##################################################
## Included modules
##################################################
## Perl modules
...
=head1 TriAnnot::Parsers::isbpFinder - Methods
=cut
...
sub _parse {
	...
	while(my $currentFeature = $gffIoObject->next_feature()){
		...
		push(@isbpFinderFeatures, $currentFeature);
	}

	$logger->debug('End of parsing (Stop at ' . localtime() . ')');

	return @isbpFinderFeatures;
}
