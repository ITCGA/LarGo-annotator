...
class FuncAnnot_IWGSC (FuncAnnot):
    ...
    def preExecutionTreatments(self):
        ...
        # Determine the number of protein to annotate to adapt the number of thread to use
        self._analyzeSequenceFile()
        self.logger.info("Number of protein to annotate: %s" % self.numberOfSequenceToAnnotate)

        # No protein case -> Cancel all tasks that depends on FuncAnnot and run FuncAnnot with only 1 thread
        if self.numberOfSequenceToAnnotate == 0:
            self.logger.info("%s didn't have any protein sequence to annotate. All depending tasks will be canceled !" % (self.getDescriptionString().capitalize()))
        ...
        # Several proteins to annotate
        # Display an error if we are on a subAnnotation with more than 1 protein to annotate
        ...
        # -> We will have to switch to the fallback runner and then we could reduce the number of thread to 1
            ...
            if self.parameters['isSubAnnotation'] == 'yes':
                ...
                self.abortPipelineReason = "%s has more than one protein to annotate in subAnnotation mode !" % (self.getDescriptionString().capitalize())
            else:
                self.parameters['nbCore'] = 1
